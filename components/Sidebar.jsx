...
import { getDocumentsByAuthor, getDocumentsByTags, getDocumentsByCategory } from "@/utils/doc-util";
                  ...
                  <ul role="list" className="border-l border-transparent">
                    {nonRootNodesGrouped[rootNode.id].map((subroot) => (
                      <li key={subroot.id} className=" relative">
                        <Link className="flex justify-between gap-2 py-1 pl-7 pr-3 text-sm text-zinc-600 transition hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white" href={`/docs/${rootNode.id}/${subroot.id}`}>
